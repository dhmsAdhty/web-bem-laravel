...
<div class="bg-white shadow-md rounded-lg p-6 w-full max-w-2xl mx-auto">
  ...
  <form action="{{ route('dashboard.anggota.update', $anggota->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    @csrf
    @method('PUT')
    ...
    <div>
      ...
      <select id="prodi" name="prodi"
        ...
        <option value="Teknik Kimia" {{ old('prodi', $anggota->prodi) == 'Teknik Kimia' ? 'selected' : '' }}>Teknik Kimia</option>
        <option value="Teknik Mesin" {{ old('prodi', $anggota->prodi) == 'Teknik Mesin' ? 'selected' : '' }}>Teknik Mesin</option>
      </select>
      @error('prodi')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
      @enderror
    ...
    <div>
      ...
      <select id="jabatan" name="jabatan"
        ...
        <option value="Ketua Departemen" {{ old('jabatan', $anggota->jabatan) == 'Ketua Departemen' ? 'selected' : '' }}>Ketua Departemen</option>
    ...
    <div>
      ...
      <select id="departemen" name="departemen"
        ...
        <option value="BPH" {{ old('departemen', $anggota->departemen) == 'BPH' ? 'selected' : '' }}>BPH</option>
        <option value="PSDM" {{ old('departemen', $anggota->departemen) == 'PSDM' ? 'selected' : '' }}>PSDM</option>
        <option value="KOMINFO" {{ old('departemen', $anggota->departemen) == 'KOMINFO' ? 'selected' : '' }}>KOMINFO</option>
        <option value="KWU" {{ old('departemen', $anggota->departemen) == 'KWU' ? 'selected' : '' }}>KWU</option>
        <option value="Internal & Eksternal" {{ old('departemen', $anggota->departemen) == 'Internal & Eksternal' ? 'selected' : '' }}>Internal & Eksternal</option>
        <option value="MIKAT" {{ old('departemen', $anggota->departemen) == 'MIKAT' ? 'selected' : '' }}>MIKAT</option>
        <option value="KASTRAT" {{ old('departemen', $anggota->departemen) == 'KASTRAT' ? 'selected' : '' }}>KASTRAT</option>
      </select>
      @error('departemen')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
      @enderror
    ...
    <div>
      ...
      @if($anggota->foto)
        ...
        <img src="{{ asset('storage/'.$anggota->foto) }}" alt="Foto Anggota" class="h-20 w-20 object-cover rounded mt-1 border">
      @endif
    </div>
    ...
    <div class="flex justify-end space-x-2">
      ...
      <button type="submit" 
              class="px-4 py-2 rounded bg-blue-600 text-white hover:bg-blue-700">Perbarui</button>
    </div>
  </form>
</div>
@endsection
