            ...
            <div>
                ...
                <div class="relative">
                    <select name="event_id" id="event_id" required
                        class="w-full appearance-none border border-slate-300 bg-slate-50 rounded-lg px-4 py-3 text-slate-700 
                                   focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out">
                        <option value="" disabled>-- Pilih salah satu event --</option>
                        @foreach($allEvents as $ev)
                        <option value="{{ $ev->id }}" {{ (old('event_id', $event->id ?? '') == $ev->id) ? 'selected' :
                            '' }}>
                            {{ $ev->title }}
                        </option>
                        @endforeach
